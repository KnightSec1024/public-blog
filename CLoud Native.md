...
## 关于日志脱敏
抛出问题：
企业在应用日志打印敏感信息这类问题的处理中，通常有两层处理手段，一种是开发一个通用组件（也可能不同的语言类型对应不同的组件），业务应用代码中需要引入该组件，该组件配合日志组件（如：log4j2）实现在日志打印语句中识别出敏感数据并对其进行脱敏。这种方式对代码有一定的侵入性，日志打印语句的性能会有一定影响，适合对性能要求不高的业务应用。好处是从根源处解决敏感打印问题，因为业务应用生成日志的过程中就已经实现的脱敏，即在应用pod上产生的日志原始文件中就不带敏感数据。

但是对与一些性能要求高的场景（如高频交易），这种集成组件就明显存在性能问题，这就需要在异步模式下实现脱敏，比如在Vector上增加一些脱敏配置项

### 什么是Vector
刚才提到的在Vector上增加脱敏配置的方式，是比较推荐的一种性能敏感场景下实现脱敏的方案。
...
graph LR
    subgraph 脱敏方案架构
        direction TB
        A["① 应用无感知"] --> B["② 异步处理"] --> C["③ 集中脱敏"] --> D["④ 数据隔离"] --> E["⑤ 按时分索引"]
        
        A -.-> A1[应用只管打日志，照常写入Kafka，不做任何改动]
        B -.-> B1[Vector 作为独立消费者，不影响应用主链路]
        C -.-> C1[敏感规则在 ConfigMap 中统一维护，规则更新只需改配置]
        D -.-> D1[Kafka 保留原始数据（可设TTL自动清除），ES 只存脱敏后数据]
        E -.-> E1[便于按时间范围查询和索引生命周期管理（ILM）]
    end
'''
### 方案对比
可选方案
'''
方案A: Kafka → Vector → ES          ← 当前方案
方案B: Kafka → Logstash → ES
方案C: Kafka → Flink/Spark → ES     (流处理框架)
方案D: 消费者应用内脱敏 → ES         (业务代码处理)
方案E: Kafka → Kafka Streams → ES   (Kafka原生流处理)
'''

方案对比
| 推荐度 | 方案 | 优点 | 缺点 |
| :---: | :--- | :--- | :--- |
| ⭐⭐⭐⭐⭐ | **Vector（当前）** | 性能极高、资源占用小、配置简单 | 复杂逻辑表达能力有限 |
| ⭐⭐⭐⭐ | **Kafka Streams** | Kafka 原生 | 需要写代码，仍需维护服务 |
| ⭐⭐⭐ | **Logstash** | 生态成熟、插件丰富 | 重、慢、吃内存（JVM） |
| ⭐⭐ | **Flink/Spark** | 超强处理能力、支持复杂逻辑 | 太重，小场景杀鸡用牛刀 |
| ⭐ | **业务代码内处理** | 灵活 | 侵入业务、各应用重复造轮子 |
